...
// GitHub Pages (Project Pages) など base path が付く環境でも画像が崩れないようにする
...
export const Image = ({ src, alt, className = "" }: { src: string, alt: string, className?: string }) => (
  <img src={assetUrl(src)} alt={alt} className={`object-cover ${className}`} referrerPolicy="no-referrer" />
);
...
export const Wireframe = () => {
  return (
    ...
    <section className="relative min-h-[85vh] flex items-center pt-20 overflow-hidden">
      ...
      >
        <Image src="/hero.jpg" alt="GOAT personal gym（泉佐野）のトレーニング風景" className="w-full h-full" />
      ...
      <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none opacity-[0.08] mix-blend-overlay">
        <img src={assetUrl("/logo-goat.png")} alt="GOAT personal gym ロゴ" className="w-[150%] md:w-[80%] max-w-none object-contain" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-8 md:px-16 flex flex-col items-center text-center">
        ...
        >
          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black text-white tracking-tighter flex flex-col items-center mb-8 drop-shadow-lg">
            <span className="leading-none">GOAT personal gym</span>
            <span className="text-sm sm:text-base md:text-lg lg:text-xl tracking-[0.3em] mt-4 font-bold opacity-90">IZUMISANO, OSAKA</span>
          </h1>
    ...
    <section id="concept" className="py-24 px-8 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none opacity-[0.02] mix-blend-multiply">
        <img src={assetUrl("/logo-goat.png")} alt="GOAT personal gym ロゴ" className="w-[120%] md:w-[60%] max-w-none object-contain" />
      </div>
    ...
    <section id="trainer" className="bg-[#F0EBE1] py-24 px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-16 items-center relative z-10">
        ...
        >
          {/* 仮画像: トレーナー 格闘家（試合風景） */}
          <Image src="/trainer.jpg" alt="GOAT personal gym トレーナー" className="w-full h-[500px] md:h-[600px] object-cover transition-transform duration-700 group-hover:scale-105" />
        </motion.div>
        <motion.div 
          initial={{ opacity: 0, x: 40 }}
          whileInView={{ opacity: 1, x: 0 }}
      ...
      <div className="max-w-6xl mx-auto mt-16 relative z-10">
        ...
        >
          <p className="text-sm font-bold text-[#6B705C] tracking-widest mb-2">GALLERY</p>
          <h3 className="text-2xl md:text-3xl font-black font-sans tracking-normal">ジムの雰囲気</h3>
          <p className="text-[#7A7268] mt-3">トレーニングの様子・空間の雰囲気が伝わる写真です。</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((n) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 0.6, delay: (n - 1) * 0.05 }}
              className="overflow-hidden rounded-[1.5rem] shadow-lg shadow-[#4A4238]/10 border border-[#E8E3D9] bg-white group"
            >
              <Image
                src={`/gallery-${n}.jpg`}
                alt={`GOAT personal gym ギャラリー ${n}`}
                className="w-full h-[220px] md:h-[240px] object-cover transition-transform duration-700 group-hover:scale-105"
              />
            </motion.div>
          ))}
        </div>
